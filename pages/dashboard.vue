<template>
  <div class="flex flex-col h-screen w-full bg-slate-900 text-slate-100">
    ...
    <!-- Map Container (Top Panel) -->
    <div class="flex-1 relative w-full h-[60vh] min-h-[400px]">
      <!-- Map -->
    ...
    <aside 
      class="flex flex-col w-full bg-slate-800 border-t border-slate-700 transition-all duration-300 ease-in-out"
    ...
    >
      ...
      <div class="p-6 flex-1 overflow-y-auto custom-scrollbar">
        <div class="max-w-5xl mx-auto w-full grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div class="space-y-4 lg:col-span-2">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <!-- Ship Name -->
              <div>
                <label class="label text-xs uppercase font-bold text-slate-400 mb-1 block">Masukkan Nama Kapal</label>
                <input v-model="shipName" type="text" class="input w-full bg-slate-900/50 border-slate-600 focus:border-primary" placeholder="Nama Kapal..."/>
              </div>

              <!-- Route Select -->
              <div>
                   <label class="label text-xs uppercase font-bold text-slate-400 mb-1 block">Saved Routes</label>
                   <div class="flex gap-2">
                     <select 
                       v-model="selectedRouteId" 
                       @change="onRouteSelect"
                       class="flex-1 px-3 py-2 bg-slate-900/50 border border-slate-600 rounded-lg text-white text-sm focus:border-primary focus:outline-none transition-colors"
                     >
                      ...
                      <button 
                       ...
                       @click="deleteRoute"
                       class="px-3 py-2 bg-red-900/30 border border-red-700/50 rounded-lg text-red-400 hover:bg-red-900/50 hover:border-red-600 transition-colors"
                       title="Delete Route"
                     >
          ...
          <div class="space-y-4">
             <div class="p-4 bg-slate-900/50 rounded-lg border border-slate-700/50 grid grid-cols-2 gap-4">
               <div>
                 <span class="block text-xs text-slate-500 mb-1 uppercase">Avg Speed</span>
                 <span class="block text-xl font-mono font-bold text-accent">{{ formattedSpeed }} <span class="text-xs font-normal text-slate-500">kts</span></span>
               </div>
               <div>
                  <span class="block text-xs text-slate-500 mb-1 uppercase">Total Distance</span>
                  <span class="block text-xl font-mono font-bold text-white">{{ totalDistance.toFixed(0) }} <span class="text-xs font-normal text-slate-500">km</span></span>
               </div>
             </div>
